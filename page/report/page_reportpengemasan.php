<div class="container">
    <h6 class="fw-bold mt-3 text-start"><img src="../asset/icon/report.png"> Input Parameter Report - Pengemasan</h6>
    <hr class="w-50 mb-3">
    ...
    <div class="form-group row mb-3">
        <label for="yearreportpengemasan" class="col-sm-2 col-form-label">Planning Years</label>
        <div class="col-sm-1">
            <input type="number" class="form-control" id="yearreportpengemasan" value="<?= date('Y') ?>" max="<?= date('Y') ?>">
        </div>
    </div>
    <div class="form-group row mb-0">
    ...
    <div class="modal-dialog modal-xl modal-dialog-scrollable">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <table id="reportpengemasan" class="table table-sm" style="width:100%;">
                    ...
                    <!-- <tbody>
                        <?php
                        $sql = mysqli_query($conn, "SELECT * FROM planning_prod_header WHERE Plant='$_SESSION[plant]' AND UnitCode='$_SESSION[unitcode]' ORDER BY Years DESC");
                        while ($row = mysqli_fetch_array($sql)) {
                        ?>
                            <tr>
                                <td><a href="#" class="badge bg-success href_transform" onclick="prosesselectreport('<?= $row['PlanningNumber'] ?>','<?= $row['Years'] ?>')"><?= $row['PlanningNumber'] ?></a></td>
                                <td><?= $row['Years'] ?></td>
                                <td><?= $row['ProductID'] ?></td>
                                <td><?= $row['ShiftID'] ?></td>
